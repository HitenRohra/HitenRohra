...
Env:
  ...
  GH_LOGIN      user to summarise (default: HitenRohra)
  OUT_DIR       where to write (default: repository root)

Adapted from the "A GitHub profile that generates itself" guide by Andrii Drok
(github.com/andriidrok1/andriidrok1).
"""
import base64
import functools
import json
...
# Section headings, rendered as SVG because GitHub strips CSS from markdown.
...
# The language column is 11 characters wide, so anything longer gets cut —
# "Jupyter Notebook" became "jupyter not". Shorten the known offenders by hand
# rather than widening the column and unbalancing the two charts.
SHORT = {"Jupyter Notebook": "notebook", "Objective-C": "obj-c",
         "Objective-C++": "obj-c++", "Vim Script": "vimscript",
         "Emacs Lisp": "elisp", "Rich Text Format": "rtf",
         "Common Lisp": "lisp", "Standard ML": "sml",
         "Visual Basic .NET": "vb.net", "Jupyter": "notebook"}


def shorten(name):
    return SHORT.get(name, name).lower()[:11]
    ...
    for gi, (gx, title, data, as_pct) in enumerate(groups):
        ...
        for ri, (name, val) in enumerate(data):
            ...
            p.append(f'<g opacity="0">{fade(0.24 + gi * 0.10 + ri * 0.05)}'
                     + label(gx, y + 8, shorten(name), 11, "e-f")
...
def main():
    ...
    login = os.environ.get("GH_LOGIN", "HitenRohra")
